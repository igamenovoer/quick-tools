...
Options:
  ...
  --output, -o arg      Output filename or directory. If a directory is specified, the video will be saved 
                        there with the default naming. If a filename is specified, the video will be saved 
                        with that name. Subtitles will follow the base name of the video file.
                        Default: "downloads" directory with video title as filename.
...
Examples:
  ...
  ./download-youtube-high-quality.sh --output "/path/to/videos" "https://www.youtube.com/watch?v=dQw4w9WgXcQ"

  # Download with a specific filename
  ./download-youtube-high-quality.sh -o "my_video.mp4" "https://www.youtube.com/watch?v=dQw4w9WgXcQ"
    ...
    case "${script_args[$i]}" in
        ...
        "--output"|"-o")
            if [[ $((i + 1)) -lt ${#script_args[@]} ]]; then
                ...
                echo "Error: --output/-o requires a value."
                exit 1
            fi
            ;;
        "--proxy")
    ...
    if [[ ${#lang_map[@]} -eq 0 ]]; then
        ...
        if [[ -n "$sub_lang_code" ]]; then
            ...
            if [[ "$embed_subtitle" == true ]]; then
                ...
                # Add subtitle-specific output template if we have a custom filename
                if [[ -n "$output_path" && "$output_path" != *"/" ]]; then
                    yt_dlp_args+=("--output" "subtitle:$subtitle_template")
                fi
...
if [[ -n "$output_path" ]]; then
    if [[ -d "$output_path" ]]; then
        echo "Download completed. Video saved in $output_path directory."
    else
        echo "Download completed. Video saved as: $output_path"
    fi
else
    echo "Download completed. Video saved in downloads directory."
fi
